...
// Full-cumulative-file text + new-line indices below were derived and
// cross-checked programmatically against the source markdown's own "Full
// file so far" checkpoints (this session's own Step 3, and Hand Session 3
// Step 1's shown context) before being transcribed here. See the
// conversion ticket for the verification script.
...
export const handSession2: Session = {
  id: "hand-s2",
  sessionNumber: 2,
  track: "Hand Tracking (2 of 4)",
  title: "Hand Tracking — Session 2 (Landmarks & Skeleton)",
  syllabusMapping: "Module 10 — Hand Tracking & Gesture Control",
  buildsOn: "Hand Tracking Session 1's webcam loop",
  sessionGoal:
    "Last time we got a mirrored camera window with nothing detected. Today we load a real hand-tracking model and draw a glowing skeleton over your actual hand.",
  instructorNotes: [
  ...
  steps: [
    {
      id: "hand-s2-1",
      index: 0,
      title: "Load the hand-tracking model",
      ...
      file: { filename: "hand_ar.py", code: step1Code, newLineIndices: [1, 4, 5, 6, 19, 34, 35, 36, 37, 38, 39, 40, 41, 42, 70] },
      commands: [],
      why: "running_mode=vision.RunningMode.VIDEO tells MediaPipe to expect a continuous stream of frames in order, rather than treating each one as an unrelated standalone image — this lets it use information between frames to track a hand more smoothly (less jitter) instead of re-detecting from scratch every time. num_hands=2 caps how many hands it looks for at once. The two confidence values are the model's certainty thresholds — how sure it must be before it reports a hand at all, and before it keeps tracking one it already found. HAND_CONNECTIONS builds the list of \"which landmark connects to which\" pairs once, from MediaPipe's own predefined hand skeleton — we'll draw a line for every pair in Step 3.",
      glossary: [
        { term: "import mediapipe as mp", explanation: "brings in MediaPipe, Google's toolkit for detecting hands, faces, and more from pictures." },
        { term: "HandLandmarker", explanation: "the specific MediaPipe tool that finds hand positions (\"landmarks\") in a picture." },
        { term: "vision.HandLandmarkerOptions(...)", explanation: "a settings bundle: all the choices you hand to HandLandmarker at once when creating it, instead of separate arguments." },
        { term: "running_mode=vision.RunningMode.VIDEO", explanation: "tells the model \"expect a stream of frames in order,\" not one unrelated photo at a time." },
    ...
    },
    {
      id: "hand-s2-2",
      index: 1,
      title: "Run detection on every frame",
      say: "Now, every single frame, we hand it to the model and get back a result — where it thinks any hands are.",
      file: { filename: "hand_ar.py", code: step2Code, newLineIndices: [62, 63, 64, 65, 66, 67] },
      commands: [],
      why: "OpenCV reads frames as BGR (blue-green-red order); MediaPipe expects RGB, so cv2.cvtColor(..., cv2.COLOR_BGR2RGB) swaps the channel order before handing the frame over — skip it and detection still runs, just less accurately, since the model was trained on RGB images. Because we're in VIDEO running mode (Step 1), detect_for_video requires an explicit, increasing timestamp with every call so MediaPipe knows how frames are ordered in time — int(time.time() * 1000) (milliseconds since epoch) is a simple always-increasing source. h, w = frame.shape[:2] grabs the frame's actual pixel height/width now, which Step 3 needs to convert MediaPipe's landmark coordinates (given as 0–1 fractions) into real on-screen pixel positions.",
      glossary: [
        { term: "cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)", explanation: "converts a picture from one color-channel order to another (here, OpenCV's blue-green-red order to MediaPipe's expected red-green-blue)." },
        { term: "mp.Image(...)", explanation: "wraps a picture in the specific format MediaPipe's functions expect to receive." },
        { term: "time.time()", explanation: "gives you the current real-world time, used here to build an always-increasing timestamp." },
        { term: "landmarker.detect_for_video(...)", explanation: "runs the hand-detection model on one frame and hands back the result." },
    ...
    },
    {
      id: "hand-s2-3",
      index: 2,
      title: "Extract landmark points and draw the skeleton connections",
    ...
    },
    {
      id: "hand-s2-4",
      index: 3,
      title: "Draw the joints",
      say: "Lines alone look like a wireframe; a small glowing dot at each of the 21 points makes the joints read clearly too.",
      file: { filename: "hand_ar.py", code: step4Code, newLineIndices: [22, 32, 33, 34, 35, 36, 92, 93] },
      commands: [],
      why: "glow_circle is the same two-layer trick as glow_line — a bigger, dimmer glow circle underneath a smaller, brighter core circle — applied to every one of the 21 points in pts, not just the connected ones, so every joint gets a dot even ones that happen to be endpoints of multiple lines.",
      glossary: [
