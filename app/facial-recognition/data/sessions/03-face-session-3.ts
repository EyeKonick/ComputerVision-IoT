...
export const faceSession3: Session = {
  id: "face-s3",
  sessionNumber: 3,
  track: "Face Tracking (3 of 4)",
  title: "Face Tracking — Session 3 (Mood Detection)",
  syllabusMapping: "Module 11 — Face Detection & Recognition (guided)",
  buildsOn: "Face Tracking Session 2's sci-fi HUD and face mesh",
  buildsToward: "source code/facial-recognition/face_tracker.py",
  sessionGoal:
    "Today the code stops just finding a face and starts reading it — we measure the mouth and decide whether the face looks happy, sad, surprised, or neutral.",
  instructorNotes: [
  ...
  steps: [
    {
      id: "face-s3-1",
      index: 0,
      title: "Measure the mouth (mouth_metrics)",
      ...
      file: {
        ...
        code: step1Code,
        newLineIndices: [0, ...range(13, 19), ...range(49, 77)],
      },
      commands: [],
      ...
      why: "The face mesh gives every landmark an x and y as a fraction of the image (0.0 to 1.0), so pt(idx) multiplies by img_w and img_h to get pixel positions. The six indices are specific numbered points on the mesh: the outer corner of each eye (33 and 263), the two mouth corners (61 and 291), and the middle of the upper and lower lip (13 and 14). math.hypot(dx, dy) is just the straight-line distance between two points. The key idea is dividing every mouth measurement by interocular (the distance between the eyes): a face close to the camera is bigger in pixels than a face far away, but the eye-to-eye distance grows and shrinks by the same amount, so \"mouth width / eye distance\" is the same number at arm's length or nose-to-lens. It turns pixels into fractions of a face. The three results are mouth_width (corner to corner), mouth_height (top lip to bottom lip), and corner_lift. corner_lift compares the middle of the mouth to the average height of the two corners — and because image y grows downward, a smile (corners higher than the center) makes mouth_center_y minus corner_avg_y positive, while a frown makes it negative. The if interocular < 1e-3: return None guard exists because dividing by a number near zero would produce garbage; returning None lets whoever calls the function skip that frame instead of crashing.",
      glossary: [],
      commonProblems: [
        {
          error: "NameError: name 'math' is not defined — but only later, in Step 3",
          solution:
            "Forgetting import math at the top doesn't fail now, because nothing calls mouth_metrics() yet. The error only appears once Step 3 starts using it, which makes it feel unrelated. If you see it later, come back to the imports.",
        },
        {
          error: "A typo in a landmark number gives no error, just a wrong mood later",
          solution:
            "Every index from 0 to 477 is a valid point on the mesh, so a mistyped one (say 16 instead of 61) still runs — it just measures the wrong spot on the face. Checking the six LM_ numbers against the list is the fastest fix.",
        },
        {
          error: "Why can corner_lift be negative?",
          solution:
            "Positive means the mouth corners sit higher than the middle of the mouth (a smile shape); negative means they sit lower (a frown shape); near zero is a flat mouth. Remember that image y grows downward, which is why the subtraction is center minus corners.",
        },
        {
          error: "Mixing up img_w and img_h inside pt()",
          solution:
            "lm.x pairs with img_w and lm.y pairs with img_h. Swapping them still runs, but every measurement is quietly distorted, and the mood readout later will feel randomly wrong.",
        },
      ],
    },
    {
      id: "face-s3-2",
      index: 1,
      title: "Decide the mood (classify_mood)",
      ...
      file: {
        filename: "face_tracker.py",
        code: step2Code,
        newLineIndices: [...range(21, 30), ...range(91, 108)],
      },
      commands: [],
      runCommand: "python face_tracker.py",
      runResult:
    ...
    },
    {
      id: "face-s3-3",
      index: 2,
      title: "Calibrate to a neutral face",
      ...
      file: {
        filename: "face_tracker.py",
        code: step3Code,
        newLineIndices: [28, ...range(112, 120), 151, ...range(154, 158), ...range(202, 213), ...range(235, 243), ...range(252, 258)],
      },
      commands: [],
      runCommand: "python face_tracker.py",
      runResult:
    ...
    },
    {
      id: "face-s3-4",
      index: 3,
      title: "Show the mood — and recalibrate with R",
      ...
      file: {
        filename: "face_tracker.py",
        code: step4Code,
        newLineIndices: [...range(33, 38), ...range(251, 258), ...range(274, 279)],
      },
      commands: [],
      runCommand: "python face_tracker.py",
      runResult:
