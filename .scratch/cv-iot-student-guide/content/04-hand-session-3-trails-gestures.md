# Session: Hand Tracking — Session 3 (Trails & Gesture Recognition)

**Track:** Hand Tracking (3 of 4)
**Maps to syllabus:** Module 10 — Hand Tracking & Gesture Control
**Builds on:** Hand Tracking Session 2's detected skeleton
**Session goal (say this out loud at the start):** "Today your hand leaves a trail of light behind each fingertip, and the screen tells you how many fingers you're holding up — and whether you're pinching."
...
## Step 1 — Fingertip particle trails
...
TRAIL_LENGTH = 18            # points remembered per fingertip trail            # ← new
FINGERTIP_IDS = [4, 8, 12, 16, 20]   # thumb, index, middle, ring, pinky tips   # ← new
...
TRAIL_COLORS = [                                                               # ← new
    (255, 80, 80),    # thumb   - blue-ish                                     # ← new
    (80, 255, 255),   # index   - yellow                                       # ← new
    (80, 255, 80),    # middle  - green                                        # ← new
    (255, 80, 255),   # ring    - magenta                                      # ← new
    (80, 160, 255),   # pinky   - orange                                       # ← new
]                                                                              # ← new
...
def main():
    ...
    # trails[hand_index][finger_index] = deque of (x, y) points                # ← new
    trails = [                                                                 # ← new
        [collections.deque(maxlen=TRAIL_LENGTH) for _ in FINGERTIP_IDS]         # ← new
        for _ in range(2)                                                      # ← new
    ]                                                                          # ← new
    ...
    while True:
        ...
        if result.hand_landmarks:
            for hand_idx, hand_landmarks in enumerate(result.hand_landmarks[:2]):
                ...
                # --- fingertip particle trails ---                            # ← new
                for f_idx, tip_id in enumerate(FINGERTIP_IDS):                  # ← new
                    trails[hand_idx][f_idx].append(pts[tip_id])                  # ← new
                    trail = trails[hand_idx][f_idx]                              # ← new
                    n = len(trail)                                              # ← new
                    for i in range(1, n):                                       # ← new
                        fade = i / n                                            # ← new
                        radius = max(1, int(6 * fade))                          # ← new
                        color = tuple(int(c * fade) for c in TRAIL_COLORS[f_idx])  # ← new
                        cv2.circle(overlay_layer, trail[i], radius, color, -1, cv2.LINE_AA)  # ← new
                    glow_circle(overlay_layer, pts[tip_id], 8, TRAIL_COLORS[f_idx], TRAIL_COLORS[f_idx])  # ← new
...
python hand_ar.py
```
Wave your hand around — you should see five short trails of colored light following each fingertip.

**New in this step:**
- `import collections` — brings in Python's built-in toolbox of extra list-like data structures.
- `collections.deque(maxlen=N)` — a list that automatically forgets its oldest item once it holds more than `N` items, like a rolling sticky-note pad.
- `trails[hand_idx][f_idx].append(...)` — adds a new item onto the end of a list (or deque).
- `range(2)` — produces the sequence of numbers `0, 1` — here, just used to repeat something twice.
- `len(trail)` — tells you how many items are currently in a list (or deque).
- `tuple(int(c * fade) for c in TRAIL_COLORS[f_idx])` (a **generator expression**) — like a list comprehension, but built one item at a time; wrapped in `tuple(...)` here to turn it into a fixed group of numbers (the color).

**Why it matters:** `trails` is a list-of-lists: one slot per possible hand (`range(2)`), and inside each, one `deque` per tracked fingertip (`FINGERTIP_IDS`). A `collections.deque(maxlen=TRAIL_LENGTH)` is a list that automatically drops its oldest item once it's full — appending a new point to a full deque quietly pushes the oldest one out, which is exactly "remember the last 18 positions" with zero manual trimming logic. Each trail point is drawn smaller and dimmer the older it is (`fade = i / n`, applied to both `radius` and `color`), which is what creates the comet-tail look instead of a solid line. `TRAIL_COLORS` gives each of the 5 fingertips its own consistent color, so you can tell fingers apart even mid-motion.
...
**New helper function (added to the Helpers section, near `glow_circle`):**
```python
import collections
import math                                                                    # ← new
import os
...
```
```python
def count_fingers(pts):                                                        # ← new
    """Return number of extended fingers (0-5) using pixel-space landmark points.  # ← new

    Distance-based rather than axis-based, so it holds up regardless of hand    # ← new
    rotation, tilt, or left/right handedness (a simple x/y-coordinate           # ← new
    comparison only works for one specific hand orientation).                  # ← new
    """                                                                        # ← new
    wrist = pts[0]                                                             # ← new
    extended = 0                                                               # ← new

    # Four fingers: tip farther from the wrist than its pip joint == extended. # ← new
    finger_tips_pips = [(8, 6), (12, 10), (16, 14), (20, 18)]                    # ← new
    for tip_id, pip_id in finger_tips_pips:                                     # ← new
        if math.hypot(pts[tip_id][0] - wrist[0], pts[tip_id][1] - wrist[1]) > \  # ← new
           math.hypot(pts[pip_id][0] - wrist[0], pts[pip_id][1] - wrist[1]):     # ← new
            extended += 1                                                       # ← new

    # Thumb: measured against the pinky's base instead, since a curled thumb   # ← new
    # can still be "far" from the wrist even when folded across the palm.      # ← new
    pinky_mcp = pts[17]                                                        # ← new
    thumb_tip_dist = math.hypot(pts[4][0] - pinky_mcp[0], pts[4][1] - pinky_mcp[1])  # ← new
    thumb_ip_dist = math.hypot(pts[3][0] - pinky_mcp[0], pts[3][1] - pinky_mcp[1])   # ← new
    if thumb_tip_dist > thumb_ip_dist:                                          # ← new
        extended += 1                                                          # ← new

    return extended                                                           # ← new
```

Not called anywhere yet — that's Step 3. Running the file now looks identical to Step 1; that's expected.

**New in this step:**
- `import math` — brings in Python's built-in toolbox of math functions.
- `math.hypot(dx, dy)` — calculates the straight-line distance between two points, given the horizontal and vertical distance between them.
- `"""..."""` (a **docstring**) — a description written right inside a function, explaining what it does; it doesn't affect how the code runs.
- `\` at the end of a line — tells Python "this statement continues on the next line," used purely to keep long lines readable.

**Why it matters:** A tempting shortcut would be "a finger is extended if its tip is *above* its base" — but that only works when the hand is held upright facing the camera; tilt or rotate the hand and it breaks immediately. Comparing *distance from the wrist* instead works no matter the hand's orientation: an extended finger's tip is simply farther from the wrist than the middle knuckle (`pip`) below it, curled or not. The thumb needs its own special case — MediaPipe's landmark numbering means a curled thumb can still measure "far from the wrist," so instead it's compared against the pinky's base (`pts[17]`), which only reads as "far" when the thumb is genuinely stuck out to the side.
...
## Step 3 — Pinch detection and the handedness label
...
**Changed lines only (inside the per-hand loop, rest of file unchanged from Step 2):**
```python
CAM_SOURCE = 0  # laptop webcam. Lab PC + phone: "http://localhost:8080/video"
FRAME_W, FRAME_H = 1280, 720
...
PINCH_THRESHOLD = 40   # pixels, normalized-ish for 1280px wide frame           # ← new

HAND_CONNECTIONS = [(c.start, c.end) for c in vision.HandLandmarksConnections.HAND_CONNECTIONS]
```
```python
            for hand_idx, hand_landmarks in enumerate(result.hand_landmarks[:2]):
                handedness_label = "Right"                                      # ← new
                if result.handedness and hand_idx < len(result.handedness):      # ← new
                    handedness_label = result.handedness[hand_idx][0].category_name  # ← new
...
```python
                ...
                # --- gesture readout ---                                       # ← new
                finger_count = count_fingers(pts)                               # ← new
                thumb_tip = pts[THUMB_TIP_ID]                                    # ← new
                index_tip = pts[INDEX_TIP_ID]                                    # ← new
                pinch_dist = math.hypot(thumb_tip[0] - index_tip[0], thumb_tip[1] - index_tip[1])  # ← new
                is_pinching = pinch_dist < PINCH_THRESHOLD                       # ← new
```

Still nothing new on screen — the values are being computed but not drawn yet. Step 4 finishes the job.

**New in this step:**
- `result.handedness` — a separate list on the detection result, holding MediaPipe's best guess of "Left" or "Right" for each hand found, lined up in the same order as `result.hand_landmarks`.
- `.category_name` — reads the actual text label (like `"Right"`) out of MediaPipe's guess.
- `and` (inside an `if` condition) — checks that *both* things on either side are true before continuing; here, "there is handedness data at all" and "there's an entry for this specific hand."

**Why it matters:** `result.handedness` is a separate list from `result.hand_landmarks`, lined up by index — `result.handedness[hand_idx][0].category_name` reads MediaPipe's own best guess of "Left" or "Right" for that hand. The default `"Right"` fallback covers the rare case where handedness data isn't available at all. Pinch detection is a straight-line distance (`math.hypot`, the same tool used inside `count_fingers`) between the thumb tip and index fingertip — smaller than `PINCH_THRESHOLD` pixels means "close enough to call it a pinch."
...
## Step 4 — Show it: the on-screen gesture readout
...
**New helper function:**
```python
def draw_text_with_bg(img, text, org, scale=0.9, color=(255, 255, 255),         # ← new
                       bg=(0, 0, 0), thickness=2, alpha=0.55):                  # ← new
    font = cv2.FONT_HERSHEY_SIMPLEX                                             # ← new
    (tw, th), baseline = cv2.getTextSize(text, font, scale, thickness)          # ← new
    x, y = org                                                                 # ← new
    overlay = img.copy()                                                       # ← new
    cv2.rectangle(overlay, (x - 10, y - th - 10), (x + tw + 10, y + baseline + 6), bg, -1)  # ← new
    cv2.addWeighted(overlay, alpha, img, 1 - alpha, 0, img)                     # ← new
    cv2.putText(img, text, (x, y), font, scale, color, thickness, cv2.LINE_AA)  # ← new
```

**Changed lines only (end of the per-hand loop, rest of file unchanged from Step 3):**
```python
                pinch_dist = math.hypot(thumb_tip[0] - index_tip[0], thumb_tip[1] - index_tip[1])
                is_pinching = pinch_dist < PINCH_THRESHOLD

                label_x = min(pts[0][0], w - 260)                                # ← new
                label_y = max(pts[0][1] + 60, 40)                                # ← new
                status = f"{handedness_label} hand: {finger_count} fingers"       # ← new
                if is_pinching:                                                  # ← new
                    status += "  |  PINCH"                                       # ← new
                draw_text_with_bg(overlay_layer, status, (label_x, label_y),     # ← new
                                   scale=0.7, color=(255, 255, 255), bg=(40, 0, 40))  # ← new
```
...
**New in this step:**
- `cv2.FONT_HERSHEY_SIMPLEX` — one of OpenCV's built-in font styles for drawing text.
- `cv2.getTextSize(...)` — measures how much space a piece of text will take up before actually drawing it, in pixels.
- `img.copy()` — makes a completely separate duplicate of a picture, so changes to the copy don't affect the original.
- `cv2.putText(...)` — draws text onto a picture at a given position.
- `f"{handedness_label} hand: {finger_count} fingers"` (an **f-string**) — builds a text string with variables' values inserted directly into it.
- `min(...)` / `max(...)` — pick the smaller or larger of two numbers; used here to keep a label from being drawn off-screen.

**Why it matters:** `draw_text_with_bg` is another two-layer trick, like `glow_line`/`glow_circle` — it draws a semi-transparent dark rectangle behind the text (via `cv2.addWeighted` blending a filled rectangle at `alpha` opacity) so the label stays readable no matter what's behind it in the live camera feed, then draws the actual text on top with `cv2.putText`. `label_x`/`label_y` position the label near the wrist (`pts[0]`) but clamped (`min`/`max`) so it never gets drawn partly off-screen when a hand is near an edge.
