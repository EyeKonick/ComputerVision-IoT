# Session: Face Tracking — Session 4 (Themes, Controls & Wrap-up)

**Track:** Face Tracking (4 of 4)
**Maps to syllabus:** Module 11 — Face Detection & Recognition (guided)
**Builds on:** Face Tracking Session 3's mood detection
**Session goal (say this out loud at the start):** "Today we finish the tracker: colour themes, on/off switches, screenshots, a proper HUD with a live FPS counter — and by the end your file is exactly face_tracker.py."

**Reminder of the code-display convention:** each step shows the entire file so far, with lines added or changed *in that step* marked `# ← new`.

> **Note for the instructor:** Say this before Step 2: some lines in this session aren't new — they're lines students already typed that have to move four spaces to the right, because Python decides what is inside an `if` by indentation. In VS Code: select the lines and press Tab. Step 4 also rewrites a few lines they already have (the window title and the end of the loop), and Step 6 is a tidy-up pass that changes nothing about behaviour.

> **Note for the instructor:** Step 4's focus fix is Windows-only (it calls the Win32 API through ctypes). On macOS or Linux it does nothing and is harmless — students there just click the window.

> **Note for the instructor:** Step 6 ends on the finished file: it should match source code/facial-recognition/face_tracker.py line for line (396 lines). That is the checkpoint students compare against.

---

## Step 1 — Colour themes (THEMES and the C key)
...
THEMES = [                                                                      # ← new
    {"name": "CYAN", "primary": (255, 255, 0), "accent": (0, 255, 255), "mesh": (255, 200, 0)},  # ← new
    {"name": "MAGENTA", "primary": (255, 0, 255), "accent": (180, 0, 255), "mesh": (255, 100, 255)},  # ← new
    {"name": "GREEN", "primary": (0, 255, 100), "accent": (0, 255, 0), "mesh": (0, 255, 150)},  # ← new
    {"name": "AMBER", "primary": (0, 200, 255), "accent": (0, 165, 255), "mesh": (0, 220, 255)},  # ← new
]                                                                               # ← new
        ...
        while True:
            ...
            if det_results.detections:
                for det in det_results.detections:
                    ...
                    draw_corner_brackets(overlay, x1, y1, x2, y2, theme["primary"])  # ← new
                    draw_scan_line(overlay, frame_count, theme["accent"], y1, y2, x1, x2)  # ← new
                    conf = det.score[0] if det.score else 0.0
                    draw_hud_text(
                        overlay, f"FACE LOCK  {conf * 100:4.1f}%",
                        (x1, max(20, y1 - 12)), theme["primary"], 0.5,          # ← new
                    )
                ...
                for landmarks in mesh_results.multi_face_landmarks:
                    mp_drawing.draw_landmarks(
                        ...
                        connection_drawing_spec=mp_drawing.DrawingSpec(
                            color=theme["mesh"], thickness=1, circle_radius=1   # ← new
                        ),
                    )
                    mp_drawing.draw_landmarks(
                        image=overlay,
                        ...
                        landmark_drawing_spec=None,
                        connection_drawing_spec=mp_drawing.DrawingSpec(
                            color=theme["primary"], thickness=1, circle_radius=1  # ← new
                        ),
                    )
            ...
            # ---- top HUD bar ----                                             # ← new
            cv2.rectangle(frame, (0, 0), (w, 40), (0, 0, 0), -1)                # ← new
            draw_hud_text(frame, "AR FACE TRACKER", (12, 26), theme["primary"], 0.65, 2)  # ← new
            draw_hud_text(frame, f"THEME: {theme['name']}", (w - 220, 26), theme["accent"], 0.5)  # ← new
            ...
            if key in (ord("q"), 27):
                ...
                theme_idx = (theme_idx + 1) % len(THEMES)                       # ← new
            elif key == ord("e"):
...
**Why it matters:** A theme is one small dictionary with four entries — a name plus three colours (primary, accent, mesh) — and THEMES is a list of four of them. theme_idx says which one is active, and (theme_idx + 1) % len(THEMES) steps forward and wraps from the last theme back to the first, the same remainder trick as the scan line. theme = THEMES[theme_idx] runs every frame, so a key press shows up on the very next frame; every place that used PRIMARY_COLOR, ACCENT_COLOR or MESH_COLOR now reads theme["primary"], theme["accent"] or theme["mesh"] instead. The first theme (CYAN) has the same colours as before, so nothing looks different until you press C. Remember OpenCV colours are blue-green-red, not red-green-blue: (255, 255, 0) is full blue plus full green, which is cyan. The top bar is a filled black rectangle (thickness -1 means "fill") drawn on frame after the blend, so it is solid instead of 85% see-through, with text drawn on top; w - 220 measures from the right edge, so the theme name stays right-aligned at any window width.

**Common problems:**
- NameError: name 'PRIMARY_COLOR' is not defined (or ACCENT_COLOR / MESH_COLOR) — The three old constants are gone, so every place that used one has to be switched to the theme. Search the file for PRIMARY_COLOR, ACCENT_COLOR and MESH_COLOR — none of them should be left.
- TypeError: list indices must be integers or slices, not str — or KeyError: 'primary' — THEMES is a list (index it with a number), while theme is a dictionary (index it with a word). Writing THEMES["primary"] gives the TypeError; a misspelled key like theme["primry"] gives the KeyError.
- "CYAN" looks yellow, or "AMBER" looks blue — or the reverse — OpenCV stores colours as (blue, green, red). If a colour looks wrong, the tuple was probably typed in red-green-blue order.
- Pressing C does nothing — The window has to have keyboard focus (click it), and Caps Lock has to be off — ord("c") only matches the lowercase letter. Step 4 adds a workaround for the focus problem.

---

## Step 2 — On/off switches for the mesh and the box (M and B)
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            if key in (ord("q"), 27):
                ...
                show_mesh = not show_mesh                                       # ← new
            elif key == ord("b"):                                               # ← new
                show_box = not show_box                                         # ← new
            elif key == ord("c"):
...
**Why it matters:** show_mesh and show_box are two True/False variables that start as True, and show_mesh = not show_mesh flips one each time its key is pressed. The drawing code isn't new: the lines you already typed are simply placed under new if show_box: and if show_mesh: lines. In Python, what belongs to an if is decided by indentation, so those existing lines shift four spaces right — nothing else about them changes. Notice what the switches do not do: detection and the mesh still run every frame, and only the drawing is skipped. That is why turning the mesh off doesn't make the program faster — and why the mood feature keeps working with the mesh hidden, since it reads the same mesh results.

**Common problems:**
- IndentationError: unexpected indent (or: expected an indented block) — The most likely mistake in this session. Every line that belongs under the new if must be indented exactly four spaces deeper than the if itself. In VS Code, select all the lines and press Tab to shift the whole block at once (Shift+Tab shifts it back).
- B hides the brackets, but the scan line or FACE LOCK label stays on screen — Only some of the lines were moved under if show_box:. All the drawing lines (brackets, scan line, the confidence value, and the label) must sit under it.
- I turned the mesh off but the FPS didn't improve — Expected. The switch only skips the drawing; the mesh model still runs on every frame (the mood feature needs it). You'll see the actual FPS number once we add the counter in Step 5.
- M or B does nothing — Same two causes as before: the window isn't focused (click it), or Caps Lock is on so the key arrives as a capital letter and ord("m") / ord("b") don't match.

---

## Step 3 — Save a screenshot (S)
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            if key in (ord("q"), 27):
                ...
                shot_count += 1                                                 # ← new
                filename = f"capture_{shot_count:02d}.png"                      # ← new
                cv2.imwrite(filename, frame)                                    # ← new
                print(f"Saved {filename}")                                      # ← new
            elif key == ord("e"):
                show_mood = not show_mood
                if show_mood:
                    mood_calibrating = True
...
**Why it matters:** shot_count counts how many screenshots this run has taken, and each press adds one. The filename f"capture_{shot_count:02d}.png" pads the number to two digits with a leading zero (01, 02, ... 10) so the files sort in the right order. cv2.imwrite(filename, frame) writes an image file, and the extension decides the format. It saves frame — the finished, composited picture with all the overlays and the top bar — not the raw camera image. The file lands in the folder your terminal is currently in (the working directory), and print confirms it. One limit worth knowing: shot_count lives only in memory, so it starts at 0 again every time you run the program.

**Common problems:**
- Screenshots from an earlier run got overwritten — The counter restarts at 0 on every run, so the first screenshot is always capture_01.png and replaces the old one. Move or rename the files you want to keep. (Checking which numbers already exist on disk is a good extension, but it isn't built here.)
- "Saved capture_01.png" printed, but I can't find the file — It was saved in the terminal's current folder, which isn't always the folder the script is in. In VS Code the integrated terminal normally opens in the project folder — check there first.
- cv2.imwrite fails but the program still prints "Saved" — imwrite returns False instead of raising an error when it can't write (a bad extension, or a folder you can't write to), and the print runs regardless. If a file never appears, this is a likely cause.
- Pressing S prints nothing at all — The key never reached the program — click the window so it has focus, and check Caps Lock. That focus problem is exactly what the next step fixes.

---

## Step 4 — Keys not responding? The window-focus fix (F)
...
WINDOW_NAME = "AR FACE TRACKER"                                                 # ← new
...
def force_window_focus(window_title):                                           # ← new
    """Windows blocks background processes from silently stealing focus, so a   # ← new
    freshly-opened cv2 window sometimes never receives keyboard input even      # ← new
    after being clicked. Explicitly activate it once via the Win32 API."""      # ← new
    try:                                                                        # ← new
        user32 = ctypes.windll.user32                                           # ← new
        hwnd = user32.FindWindowW(None, window_title)                           # ← new
        if hwnd:                                                                # ← new
            user32.ShowWindow(hwnd, 5)  # SW_SHOW                               # ← new
            user32.SetForegroundWindow(hwnd)                                    # ← new
...
def main():
    ...
    cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_NORMAL)                             # ← new
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            cv2.imshow(WINDOW_NAME, frame)                                      # ← new

            if frame_count == 5:                                                # ← new
                # Give the window a few frames to exist before grabbing focus.  # ← new
                force_window_focus(WINDOW_NAME)                                 # ← new
...
**Why it matters:** Windows stops background programs from stealing focus, so a freshly opened window can sit there never receiving keyboard input. ctypes lets Python call functions from Windows' own system libraries, and ctypes.windll.user32 is the part that manages windows on screen. FindWindowW(None, window_title) looks up a window by its exact title and returns a handle (hwnd) to it — which is why the title is now one constant, WINDOW_NAME, used both when the window is created and when it's searched for; they must match exactly. If it's found, ShowWindow(hwnd, 5) makes sure it's visible (5 is the Windows code for "show") and SetForegroundWindow(hwnd) brings it to the front with focus. The whole thing sits in try/except Exception: pass: on macOS or Linux there is no windll, so the call would fail — the except swallows that and the function quietly does nothing. cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_NORMAL) creates the window up front, and WINDOW_NORMAL makes it resizable by dragging its edges. The automatic call runs when frame_count == 5 — a few frames in, so the window certainly exists — and because it's ==, not >=, it happens only once. The blank lines added around it are just for readability.

**Common problems:**
- Nothing changes on macOS or Linux — Expected: the workaround is Windows-only (ctypes.windll doesn't exist elsewhere), and the try/except hides that instead of crashing. On other systems, click the window once to give it focus.
- A second window appears, or the focus fix never seems to work — The title used by cv2.imshow doesn't match WINDOW_NAME, so the program created two differently-named windows, or FindWindowW can't find the one it's looking for (and fails silently). Both cv2.namedWindow and cv2.imshow must use WINDOW_NAME.
- Keys are dead and pressing F doesn't help either — F is read by the same window, so it can only help while the window still receives some key presses. If nothing responds at all, click the window first — after that, F is a quick way to get focus back next time.
- AttributeError: module 'ctypes' has no attribute 'windll' — This means the ctypes.windll line ended up outside the try block. On non-Windows systems it must stay inside try so the except can catch it.

---

## Step 5 — The bottom bar: FPS, face count and key legend
...
def main():
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            if det_results.detections:
                face_count = len(det_results.detections)                        # ← new
                for det in det_results.detections:
                    box = det.location_data.relative_bounding_box
                    x1 = max(0, int(box.xmin * w) - 15)
                    y1 = max(0, int(box.ymin * h) - 15)
            ...
            # ---- bottom HUD bar ----                                          # ← new
            now = time.time()                                                   # ← new
            dt = now - prev_time                                                # ← new
            prev_time = now                                                     # ← new
            if dt > 0:                                                          # ← new
                fps = 0.9 * fps + 0.1 * (1.0 / dt)                              # ← new

            cv2.rectangle(frame, (0, h - 34), (w, h), (0, 0, 0), -1)            # ← new
            draw_hud_text(frame, f"FPS: {fps:4.1f}", (12, h - 12), theme["accent"])  # ← new
            draw_hud_text(frame, f"FACES: {face_count}", (150, h - 12), theme["accent"])  # ← new
            mood_tag = "ON" if show_mood else "OFF"                             # ← new
            draw_hud_text(                                                      # ← new
                frame,                                                          # ← new
                f"[M] mesh [B] box [E] mood:{mood_tag} [R] recalib [C] color [S] shot [F] focus [Q] quit",  # ← new
                (w - 700, h - 12), (200, 200, 200), 0.45,                       # ← new
            )                                                                   # ← new
...
**Why it matters:** dt is how many seconds passed since the previous frame, so 1.0 / dt is the speed of this one frame. Shown raw, that number jumps around (18, 27, 21, 25...) and is impossible to read. So the code keeps 90% of the old value and blends in only 10% of the new one — fps = 0.9 * fps + 0.1 * (1.0 / dt) — a running average that moves smoothly. It also explains why fps starts at 0.0 and visibly climbs at the beginning: it's averaging its way up from zero. The if dt > 0 guard avoids dividing by zero in the rare case the clock gives the same time twice. face_count is reset to 0 at the start of every frame and then set to the number of detections, so it correctly drops back to 0 when nobody is in view. mood_tag = "ON" if show_mood else "OFF" is a one-line choice between two words. The bar is drawn after the blend, so it's solid. This is also why Session 3's label is kept above the bottom edge: min(h - 45, ...) leaves room for a 34-pixel bar. Finally, this is where you can see what running two MediaPipe models costs — the number here is the real answer to "is this fast enough?".

**Common problems:**
- FPS stays at 0.0 or resets constantly — prev_time and fps must be created once, before the loop. If they're typed inside the loop they reset every frame, and the average never has anything to build on.
- The FPS number jumps around and is hard to read — The two weights are probably swapped (0.1 on the old value and 0.9 on the new one), which makes the number mostly raw. The old value should get the big share: 0.9 * fps + 0.1 * (1.0 / dt).
- FPS is much lower than expected (10-15) — Not a bug. Two MediaPipe models run on every frame, and a phone stream over USB adds its own delay. This is the slowdown Session 2 warned about, now visible.
- The key list on the right is cut off — It's positioned with w - 700, which assumes the window is at least about 700 pixels wide. Dragging the window narrower clips it.

---

## Step 6 — Tidy up: the finished face_tracker.py
...
```python
"""                                                                             # ← new
AR Face Tracker - a classroom demo.                                             # ← new

Real-time face tracking with a sci-fi HUD overlay: a glowing face mesh,         # ← new
a targeting-reticle bounding box, and a live FPS counter.                       # ← new

Controls:                                                                       # ← new
    M - toggle mesh overlay                                                     # ← new
    B - toggle bounding box / reticle                                           # ← new
    E - toggle mood detection (off by default)                                  # ← new
    R - recalibrate mood baseline (hold a neutral face when you press it)       # ← new
    C - cycle HUD color theme                                                   # ← new
    S - save a screenshot                                                       # ← new
    F - re-focus the window (use this if keypresses stop responding)            # ← new
    Q / ESC - quit                                                              # ← new
"""                                                                             # ← new
...
CAM_SOURCE = 0                                                                  # ← new
# Laptop with a built-in webcam: leave as the device index above (0).           # ← new
# Lab PC with no webcam: set to your phone's IP Webcam stream tunneled over     # ← new
# USB via `adb forward tcp:8080 tcp:8080`, e.g.:                                # ← new
#   CAM_SOURCE = "http://localhost:8080/video"                                  # ← new
...
def main():
    ...
    if isinstance(CAM_SOURCE, int):
        ...
        cap = cv2.VideoCapture(CAM_SOURCE)  # URL source: let OpenCV pick the FFmpeg backend  # ← new
    if not cap.isOpened():
        raise RuntimeError("Could not open webcam. Check CAM_SOURCE / camera permissions.")
...
**Why it matters:** The block of text in triple quotes at the very top is a docstring — the file's own description. It's ignored when the program runs, but tools and other programmers read it, and here it doubles as a cheat-sheet of every key. The banner comments split the file into Config, Helpers and Main, and the longer CAM_SOURCE comment explains what to change on a lab PC (a phone tunnelled over USB). The comment beside the URL branch tells the reader why it's a separate case. Swapping the two process() lines makes no difference to the result: face detection and the face mesh both read the same rgb picture and neither uses the other's answer, so their order doesn't matter — the finished file just happens to run the mesh first. Step back and look at the whole file: constants at the top, drawing helpers, the measuring and classifying helpers, and then main(), which does the same five things every frame — capture, detect and measure, draw, add the HUD, then read the keyboard.

**Common problems:**
- SyntaxError: unterminated triple-quoted string literal — The docstring at the top opened with three quote marks but never closed with three. Check that the closing """ is on its own line after the Q / ESC line.
- The program broke after adding comments — Comments can't break code — so a line was probably deleted or changed by accident while editing. Compare the top of your file and each changed place carefully.
- My file has a different number of lines — Blank lines count. Compare section by section (docstring, Config, Helpers, Main) to find where the counts diverge — often it's a banner comment that is missing its blank lines.
- Does the order of the two process() calls really not matter? — Correct — neither model reads the other's output, so they can run in either order and give the same results.

---

## Closing framing note (say this before the wrap-up)
