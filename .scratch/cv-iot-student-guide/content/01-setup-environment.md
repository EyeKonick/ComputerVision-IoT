...
- `cap.read()` — grabs one picture from the camera right now.
- `cv2.imshow(...)` — opens a window on your screen and shows a picture in it.
- `cv2.waitKey(0)` — pauses the program until you press a key, while keeping the window responsive while it waits.
