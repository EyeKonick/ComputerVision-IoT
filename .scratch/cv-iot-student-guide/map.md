...
- [Setup & Environment session](issues/01-setup-environment-session.md) — drafted content (Python/PATH → venv → activate → verify → install pinned deps → choose camera source → "Hello, Camera" check); also fixed a real mediapipe version mismatch in the repo (`hand-tracking/requirements.txt` now pins `==0.10.14` to match `facial-recognition/requirements.txt`). Revised after [Camera source for lab computers](issues/11-camera-source-lab-computers.md) closed — see that ticket for what changed. *Revised again:* Step 7's `hello_webcam.py` now loops continuously (`while True` + `cv2.waitKey(1)` + `cv2.flip`) instead of showing one still frame via `cv2.waitKey(0)`, since students were reading the old single-frame behavior as their camera lagging/freezing. Common-problems list expanded with the case-sensitivity (`cv2.CAP_DSHOW`/`waitKey`/`destroyAllWindows`), single-frame, inverted-`break`-condition, and mirrored-feed mistakes students actually hit building this step live, plus the VS Code/Pylance interpreter-mismatch editor note. *Revised a third time:* the `==0.10.14` pin was dropped — both `requirements.txt` files and Step 5's `pip install` command now install mediapipe unpinned, trading exact cross-machine version reproducibility for avoiding the "no matching distribution" failure on Python versions the pinned release didn't support. *Revised a fourth time:* Step 6's Path B (phone over USB) had the adb tunnel direction backwards — `adb reverse` opens its listener on the phone's own port 8080, which IP Webcam's server already occupies, so it failed with "address already in use" every time. Corrected to `adb forward` (listener on the free PC side) everywhere it appears, plus real debugging-toggle-order and screen-timeout gotchas from a live troubleshooting runbook.
